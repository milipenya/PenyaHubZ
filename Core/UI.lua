...
local function createFunctionButton(
    ...
    if miniGames then
        initialState =
            miniGames:IsEnabled(name)
    end

    UI.ToggleStates[name] =
        initialState
        ...
        function(state)
            ...
            if miniGames then
                ...
                if state then
                    ...
                    miniGames:Enable(name)

                else

                    miniGames:Disable(name)
...
function UI:Init(context)
    ...
    functionContainer.BackgroundTransparency =
        1
